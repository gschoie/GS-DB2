...
function toneEventRow(e){const cls=/상향|↑/.test(e.type)?'up':/하향|↓/.test(e.type)?'down':/의견|포인트/.test(e.type)?'mid':'';return `<button type="button" class="tone-event ${cls} tone-ck" data-ck="${esc(e.company_key)}" title="${esc(e.evidence||e.detail)}"><small>${mmdd(e.date)}</small><em>${esc(e.type)}</em><b>${esc(e.company)}</b><span>${esc(e.detail)}</span><i>${esc(e.analyst)}</i></button>`}
function renderToneEvents(){const box=$('#tone-events');if(!box)return;const events=state.tone2?.events||[];if(!events.length){box.innerHTML='';return}box.innerHTML=`<details class="tone-event-wrap" open><summary>⚡ 최근 변화 이벤트 <em>${events.length}건 · 120일</em></summary><div class="tone-event-list">${events.slice(0,30).map(toneEventRow).join('')}</div></details>`}
