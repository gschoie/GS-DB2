...
2. ## 매크로·원재료 — 매크로표의 확정값으로 금리·환율과 주요 원재료(금·은·구리·유가)의 1D 등락 및 1M/3M/12M 추세를 정리하고, 건설기계 수요 관점의 함의(금리→주택, 금·은·구리→광산 capex)를 한 줄씩. 먼저 헤더 줄 "지표 | 값(기준일) | 1D | 1M | 3M | 12M"를 쓰고, 그 아래 각 지표 행을 같은 형태의 일반 텍스트 줄로. 뉴스 목록에 주택착공·굴착기 판매 등 지표 기사가 있으면 함께.
3. ## 주요 주가 동향 — 시세표의 그룹 구분(**한국 / 글로벌 대형 / 중소형 / 광산장비 / 렌탈·딜러 / 중국**)을 그대로 따라, 그룹명을 굵은 줄로 쓰고, 바로 아래에 헤더 줄 "기업명 | 1D | 1M | 3M | 12M | 변동 요인"를 한 줄 쓴 뒤, 각 종목 행을 같은 형태의 일반 텍스트 줄로 쓰세요 (마크다운 표 문법 |---| 사용 금지). 수치는 시세표 값 그대로. 한국 그룹은 전 종목 필수, 다른 그룹은 변동폭이 크거나 뉴스가 있는 종목 위주. 변동 요인이 없으면 1M/3M/12M 추세로 짧게 코멘트.
...
def collect_only(now: datetime) -> None:
    ...
    payload = {
        ...
        "format_note": (
            "매크로·원재료 섹션: 첫 줄에 헤더 '지표 | 값(기준일) | 1D | 1M | 3M | 12M'를 쓰고 각 지표를 같은 형태 줄로, "
            "금·은·구리 추세의 광산 capex 함의 포함. "
            "주요 주가 동향 섹션: 시세표의 그룹([한국]/[글로벌 대형]/[중소형]/[광산장비]/"
            "[렌탈·딜러]/[중국]) 순서를 그대로 따라 그룹명을 굵은 줄로 쓰고, "
            "각 그룹명 바로 아래에 헤더 줄 '기업명 | 1D | 1M | 3M | 12M | 변동 요인'를 넣은 뒤 각 종목 행을 같은 형태로. "
            "한국 그룹은 전 종목 필수."
        ),
    }
