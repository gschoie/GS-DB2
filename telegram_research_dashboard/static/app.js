...
async function load(){
 ...
 const nbe=$('#construction-brief');if(nbe){const nb=window.__DASHBOARD_DATA__?.constructionBrief;if(nb&&nb.summary){const body=briefBody(nb.summary);nbe.innerHTML=`<div class="brief-head"><small>🏗️ 글로벌 건설기계 데일리 브리핑${nb.date?' · '+esc(nb.date)+(d=>isNaN(d)?'':`(${'일월화수목금토'[d.getDay()]})`)(new Date(nb.date+'T00:00:00')):''}</small><a href="construction_briefing_report.html" target="_blank" rel="noopener">전체 브리핑 →</a></div><div class="brief-body">${body}</div>`;nbe.style.display='';}else{nbe.style.display='none';}}
