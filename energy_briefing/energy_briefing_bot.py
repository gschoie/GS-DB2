...
NEWS_QUERIES = [
    ...
    # 미국 데이터센터 법안·규제 강화와 반대 움직임 — 주(州)·지방 법안, 모라토리엄,
    # 전기요금 전가 반발, 주민·환경 반대·소송 (유저 요청 축: DC 규제 리스크)
    ('(("data center" OR datacenter) AND (bill OR legislation OR regulation '
     'OR moratorium OR zoning OR ordinance OR opposition OR backlash OR protest '
     'OR lawsuit OR ratepayer OR "rate payer" OR "utility rates" OR "electric bills" '
     'OR "tax break" OR "tax incentive" OR repeal OR "public hearing")) when:1d',
     "en-US", "US", "US:en"),
...
추적 주제는 일곱 가지입니다: ① 친환경 선박 추진 기술(암모니아 추진·액화수소 추진·원자력/SMR 추진)
...
⑦ 미국 데이터센터 법안·규제 강화와 반대 움직임(연방·주·지방 법안, 모라토리엄·조닝, 전기요금
전가 논란, 주민·환경 반대와 소송, 세제혜택 축소) — 중요 꼭지로 취급
...
- 여섯 주제와 무관한 기사(비료용 암모니아, 수소차 등)는 버리세요. 데이터센터는 **투자·건설·전력 조달·법안/규제·반대 움직임** 동향을 다루고, 단순 서비스·장애·보안 기사는 버리세요.
...
- **뉴스의 함의를 끝까지 해석하세요.** FID 승인 → 신조 발주(LNG운반선·FLNG) 경로, 미국 전력난·데이터센터 수요 → 가스터빈/SMR/해상 발전 경로, 데이터센터 투자 확대 → 전력 조달(가스·SMR·PPA)·FDC 대안 경로, 미국 DC 규제·반대 강화 → 육상 입지 제약 → FDC(해상)·입지 다변화 경로, 친환경 연료 규제(IMO) → 이중연료 신조 교체 수요 경로를 짚고, 한국 조선 3사(HD한국조선해양·한화오션·삼성중공업)와 기자재 업체에 수혜·경쟁 관점을 연결하세요.
...
7. ## 미국 DC 법안·규제 & 반대 움직임 — 연방·주·지방의 데이터센터 관련 법안(강화·완화 모두)과 모라토리엄·조닝 결정, 전기요금 전가 논란, 주민·환경단체 반대·소송, 세제혜택 축소 움직임. **중요 꼭지 — 관련 보도가 있으면 반드시 포함**하고 지역(주 이름)을 명시. 없으면 "- 특이사항 없음."
8. ## 한국 조선·기자재 시사점 — HD한국조선해양·한화오션·삼성중공업 및 기자재 관점의 수혜·경쟁 포인트 3개 이내
