...
function overviewTone(r){const op=r.opinion&&!['명시 없음','없음',''].includes(r.opinion)?`<span class="tag">${esc(r.opinion)}</span>`:'';const tp=r.tp_dir==='상향'?'<span class="tag up">TP▲</span>':r.tp_dir==='하향'?'<span class="tag down">TP▼</span>':'';return `<a class="mini-line" href="${esc(r.pdf_url||r.post_url||'#')}" target="_blank" rel="noopener"><small>${esc(String(r.date||'').slice(5))}</small><b>${esc(r.company||'')}</b><span>${esc(r.title||'')}</span>${op}${tp}</a>`}
