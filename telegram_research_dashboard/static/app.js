...
async function load(){
 ...
 const nbe=$('#construction-brief');if(nbe){const nb=window.__DASHBOARD_DATA__?.constructionBrief;if(nb&&nb.summary){const nbLabel=nb.unified?'🏗️ 건기 통합 브리핑':'🏗️ 글로벌 건설기계 데일리 브리핑';const nbLink=nb.unified?'construction_unified_report.html':'construction_briefing_report.html';const body=nb.summary.split('\n').map(l=>l.trim()).filter(Boolean).map(l=>l.startsWith('## ')?'━ '+l.replace(/^#+\s*/,'').split('—')[0].trim():l.replace(/^\*+\s+/,'• ')).join('\n');nbe.innerHTML=`<div class="brief-head"><small>${nbLabel}${nb.date?' · '+esc(nb.date)+(d=>isNaN(d)?'':`(${'일월화수목금토'[d.getDay()]})`)(new Date(nb.date+'T00:00:00')):''}</small><a href="${nbLink}" target="_blank" rel="noopener">전체 브리핑 →</a></div><div class="brief-body">${esc(body).replace(/\n/g,'<br>')}</div>`;nbe.style.display='';}else{nbe.style.display='none';}}
