...
// 브리핑 카드 본문. 구글뉴스 RSS 주소가 본문의 절반을 차지해 읽기 어려웠다 —
// 마크다운 링크 [출처](주소)는 주소를 감추고 '출처 ↗' 링크만 남긴다.
function briefBody(md){const txt=md.split('\n').map(l=>l.trim()).filter(Boolean)
  .map(l=>l.startsWith('## ')?'━ '+l.replace(/^#+\s*/,'').split('—')[0].trim():l.replace(/^\*+\s+/,'• ')).join('\n');
 return esc(txt).replace(/\[([^\]\n]+)\]\((https?:\/\/[^\s)]+)\)/g,
   (m,t,u)=>`<a class="brief-src" href="${u}" target="_blank" rel="noopener">${t} ↗</a>`).replace(/\n/g,'<br>')}
...
async function load(){
 ...
 fetchLiveMacro();
 const brief=$('#daily-brief');if(brief){const gm='https://gemini.google.com/app/dc1cee4fd9194007?usp=sharing';const body=macro.daily_brief?esc(macro.daily_brief).replace(/\n/g,'<br>'):'<span class="brief-empty">아직 핵심요약이 없습니다. GMN.글로벌방산 문서에 붙여넣으면 여기 표시됩니다.</span>';brief.innerHTML=`<div class="brief-head"><small>GEMINI · 방산 브리핑</small><a href="${gm}" target="_blank" rel="noopener">Gemini 열기 →</a></div><div class="brief-body">${body}</div>`}
 // GPT 방산 브리핑 — 요약은 chatgpt_defense/<날짜>.md에서, 링크는 원본 ChatGPT 대화로.
 const gpe=$('#gpt-brief');if(gpe){const gp=window.__DASHBOARD_DATA__?.gptBrief,cu='https://chatgpt.com/c/6aaa8e3b-6ad0-83ee-9f40-5bc507a3e4d5';
  const body=gp&&gp.summary?briefBody(gp.summary)
   :'<span class="brief-empty">아직 오늘 브리핑이 없습니다. ChatGPT 대화에서 확인하세요.</span>';
  gpe.innerHTML=`<div class="brief-head"><small>🧠 GPT · 방산 브리핑${gp&&gp.date?' · '+esc(gp.date)+(d=>isNaN(d)?'':`(${'일월화수목금토'[d.getDay()]})`)(new Date(gp.date+'T00:00:00')):''}</small><a href="${cu}" target="_blank" rel="noopener">ChatGPT 열기 →</a></div><div class="brief-body">${body}</div>`}
 // 오늘의 일정 — Gemini Spark 대화로 바로 가는 칸(대시보드에 저장하는 데이터는 없다).
 const tpe=$('#today-plan');if(tpe){const sp='https://gemini.google.com/spark/chat/8d6b81ec6a0e6173?usp=sharing';
  tpe.innerHTML=`<div class="brief-head"><small>🗓️ 오늘의 일정</small><a href="${sp}" target="_blank" rel="noopener">일정 열기 →</a></div>`+
   `<div class="brief-body"><span class="brief-empty">Gemini Spark에서 오늘 일정을 확인합니다.</span></div>`}
 const ube=$('#unified-brief');if(ube){const ub=window.__DASHBOARD_DATA__?.unifiedBrief;if(ub&&ub.summary){const body=briefBody(ub.summary);ube.innerHTML=`<div class="brief-head"><small>🛡️ 방산 통합 브리핑${ub.date?' · '+esc(ub.date)+(d=>isNaN(d)?'':`(${'일월화수목금토'[d.getDay()]})`)(new Date(ub.date+'T00:00:00')):''}</small><a href="defense_unified_report.html" target="_blank" rel="noopener">전체 브리핑 →</a></div><div class="brief-body">${body}</div>`;ube.style.display='';}else{ube.style.display='none';}}

 const nbe=$('#construction-brief');if(nbe){const nb=window.__DASHBOARD_DATA__?.constructionBrief;if(nb&&nb.summary){const nbLabel=nb.unified?'🏗️ 건기 통합 브리핑':'🏗️ 글로벌 건설기계 데일리 브리핑';const nbLink=nb.unified?'construction_unified_report.html':'construction_briefing_report.html';const body=briefBody(nb.summary);nbe.innerHTML=`<div class="brief-head"><small>${nbLabel}${nb.date?' · '+esc(nb.date)+(d=>isNaN(d)?'':`(${'일월화수목금토'[d.getDay()]})`)(new Date(nb.date+'T00:00:00')):''}</small><a href="${nbLink}" target="_blank" rel="noopener">전체 브리핑 →</a></div><div class="brief-body">${body}</div>`;nbe.style.display='';}else{nbe.style.display='none';}}
 const ebe=$('#energy-brief');if(ebe){const eb=window.__DASHBOARD_DATA__?.energyBrief;if(eb&&eb.summary){const body=briefBody(eb.summary);ebe.innerHTML=`<div class="brief-head"><small>🌊 친환경 에너지·FDC 브리핑${eb.date?' · '+esc(eb.date)+(d=>isNaN(d)?'':`(${'일월화수목금토'[d.getDay()]})`)(new Date(eb.date+'T00:00:00')):''}</small><a href="energy_briefing_report.html" target="_blank" rel="noopener">전체 브리핑 →</a></div><div class="brief-body">${body}</div>`;ebe.style.display='';}else{ebe.style.display='none';}}
