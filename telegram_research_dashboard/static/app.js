...
async function load(){
 ...
 const ube=$('#unified-brief');if(ube){const ub=window.__DASHBOARD_DATA__?.unifiedBrief;if(ub&&ub.summary){const body=ub.summary.split('\n').map(l=>l.trim()).filter(Boolean).map(l=>l.startsWith('## ')?'━ '+l.replace(/^#+\s*/,'').split('—')[0].trim():l.replace(/^\*+\s+/,'• ')).join('\n');ube.innerHTML=`<div class="brief-head"><small>🛡️ 방산 통합 브리핑${ub.date?' · '+esc(ub.date)+(d=>isNaN(d)?'':`(${'일월화수목금토'[d.getDay()]})`)(new Date(ub.date+'T00:00:00')):''}</small><a href="defense_unified_report.html" target="_blank" rel="noopener">전체 브리핑 →</a></div><div class="brief-body">${esc(body).replace(/\n/g,'<br>')}</div>`;ube.style.display='';}else{ube.style.display='none';}}
