...
def build() -> Path:
    ...
    for dc_tracker_file in ("dc_tracker.html", "dc_tracker.json"):
        src = ROOT / "static" / dc_tracker_file
        if src.exists():
            shutil.copy2(src, OUTPUT.parent / dc_tracker_file)
    ...
    weekly_yt_dir = ROOT / "static" / "youtube_weekly"
    if weekly_yt_dir.is_dir():
        shutil.copytree(weekly_yt_dir, OUTPUT.parent / "youtube_weekly", dirs_exist_ok=True)
