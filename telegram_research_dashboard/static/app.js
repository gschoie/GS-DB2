...
function toneDetail(title,items,cls=''){return items?.length?`<div class="tone-report-point ${cls}"><b>${esc(title)}</b>${items.map(x=>`<p>${esc(x)}</p>`).join('')}</div>`:''}
function toneReport(r){const pitch=r.pitch?`<div class="tone-report-point"><b>Pitch</b>${esc(r.pitch)}</div>`:'',conclusion=(r.report_conclusion||r.industry_conclusion)?`<div class="tone-report-point conclusion"><b>결론 / 요약</b>${esc(r.report_conclusion||r.industry_conclusion)}</div>`:'',valuation=toneDetail('Valuation 방법',r.valuation,'valuation'),earnings=toneDetail('실적 추정 변경',r.earnings_changes,'earnings'),preferred=toneDetail('선호주',r.preferred_stocks||r.top_picks,'preferred'),changes=(r.tp_changes||r.tp_raises||[]).map(toneTp).join(''),sourceClass=r.analysis_source==='PDF 원문 분석'?'pdf':'fallback';return `<div class="tone-report"><div class="tone-report-head"><span class="tone-chip">${esc(r.report_type||'보고서')}</span><span class="tone-chip tone-source-${sourceClass}">${esc(r.analysis_source||'텔레그램 요약')}</span><b>${esc(r.date)} · ${esc(r.company)}</b></div><strong>${esc(r.title)}</strong>${pitch}${conclusion}${valuation}${earnings}${preferred}${changes}<a href="${esc(r.pdf_url||r.source_url)}" target="_blank" rel="noopener">보고서 ↗</a></div>`}
function renderTone(){const data=state.tone||{months:[]},q=state.toneQ.toLocaleLowerCase(),months=[...(data.months||[])].sort((a,b)=>b.month.localeCompare(a.month));let html='';for(const month of months){if(state.toneMonth&&month.month!==state.toneMonth)continue;let cards='';for(const a of month.analysts||[]){if(state.toneAnalyst&&a.analyst!==state.toneAnalyst)continue;if(q&&!JSON.stringify(a).toLocaleLowerCase().includes(q))continue;const picks=a.top_picks?.length?a.top_picks.map(tonePick).join(''):'<div class="tone-empty">명시 없음</div>',tps=(a.tp_changes||a.tp_raises||[]).length?(a.tp_changes||a.tp_raises).map(toneTp).join(''):'<div class="tone-empty">TP 변경 명시 없음</div>',reports=(a.reports||[]).map(toneReport).join('');cards+=`<article class="tone-card"><header><h3>${esc(a.analyst)} <small>${(a.sectors||[]).map(esc).join(' · ')}</small></h3><div class="tone-chips"><span class="tone-chip opinion-${esc(a.opinion)}">${esc(a.opinion)}</span><span class="tone-chip">월간 ${a.report_count}건</span></div></header><section class="tone-section"><h4>월간 섹터 선호주 / TOP PICK</h4><div class="tone-change">${esc(a.top_pick_change)}</div>${picks}</section><section class="tone-section"><h4>월간 TP 변경 종합</h4>${tps}</section><details><summary>이달 발간 자료 ${a.report_count}건 · Pitch / 산업 결론</summary>${reports}</details></article>`}if(cards){const openMonth=state.toneMonth?month.month===state.toneMonth:month===months[0];html+=`<details class="tone-month" ${openMonth?'open':''}><summary class="tone-month-title"><h3>${esc(month.month)}</h3><span>${month.report_count}건</span></summary><div class="tone-grid">${cards}</div></details>`}}$('#tone-content').innerHTML=html||'<div class="tone-loading">조건에 맞는 자료가 없습니다.</div>'}
...
function view(id){$$('.view,.nav').forEach(x=>x.classList.remove('active'));$('#'+id).classList.add('active');$(`.nav[data-view="${id}"]`).classList.add('active');$('#page-title').textContent={overview:'오늘의 리서치 흐름',reports:'발간 보고서',news:'뉴스.아카이브',press:'보도기사 취합','union-board':'현중 노조게시판',tone:'DAOL 리서치 톤',tasklist:'수명 피드백 확인',dart:'조선 수주공시 → 텔레',etf:'ETF/섹터 신호 포착',consensus:'코스피200 컨센서스 추적'}[id];if(id==='overview')fetchLiveMacro();if(id==='press')renderPressTable();if(id==='tone')loadTone(true);if(id==='union-board')loadUnionBoard();if(id==='tasklist'){renderTaskList();taskPull().then(ok=>{if(ok)renderTaskList()})}if(id==='etf'){const f=$('#etf-frame');if(!f.getAttribute('src'))f.src='etf_signal_report.html?t='+Date.now()}if(id==='consensus'){const f=$('#consensus-frame');if(!f.getAttribute('src'))f.src='consensus_revision.html?t='+Date.now()}}
