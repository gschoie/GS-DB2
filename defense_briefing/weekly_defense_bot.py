...
매주 토요일 KST 08:00 실행. 지난 7일의 아침 브리핑 마크다운 원문을 모아
...
# 일간 봇의 유니버스·렌더러·텔레그램 헬퍼를 재사용한다 (같은 폴더)
...
def send_weekly_telegram(md_report: str, period_label: str, now: datetime) -> None:
    """주간 정리 요약 + 링크를 텔레그램으로 발송 (일간 방산 봇과 같은 채널)."""
    ...
    header = f"🗓️ <b>글로벌 방산 주간 정리</b> | {period_label}\n"
    footer = (f'\n\n📊 <a href="{base_url}/defense_weekly/{date_str}.html">'
              "전체 주간 정리 보기 (계약·주가·시사점)</a>\n"
...
<div class="bar">
  <h1>🗓️ 글로벌 방산 주간 정리 <small style="font-size:11px;color:#8b96a8">매주 토 08:00</small></h1>
...
else fr.srcdoc='<body style="background:#0d1117;color:#8b96a8;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0">아직 주간 정리가 없습니다 — 매주 토요일 08:00에 자동 생성됩니다.</body>';
</script></body></html>"""
