...
- **크론 정시성 — GAS 시계로 이관 검토** (8/28 제안, 사용자 검토 중):
  `market-trend`(예정 KST 06:30) 실측 15회 — 8/13~8/25는 평균 +19분이었으나
  **8/27 +3시간 26분(09:56 도착), 8/28 +8시간 1분(14:31 도착)**으로 악화.
  원인은 무료 러너 `schedule`의 최저 우선순위 큐 + 이 계정의 크론 워크플로 15개
  (source-watcher 하루 45회 등)로 인한 스케줄 부하. 크론을 앞당겨도 해결 불가.
  - 제안(A안, 추천): **기존 Apps Script 프로젝트에 5분 간격 시간 트리거**를 두고
    KST 시계로 `workflow_dispatch` 발사(±5분). `GH_TOKEN`·`dispatch_proxy` WF 매핑을
    그대로 재사용 — 새 인프라·토큰 복사 없음. 대안 B(Cloudflare Workers 크론),
    C(cron-job.org).
  - **중복 방지가 전제**: GAS가 정시에 쏘고 GitHub 크론이 뒤늦게 또 돌면 텔레그램이
    두 번 간다. 워크플로에 "오늘 산출물 있으면 스킵"을 **schedule 이벤트에만** 걸어
    GAS(dispatch)를 주 경로로, GitHub 크론을 안전망으로 남길 것. 대시보드 수동 버튼은
    항상 강제 실행.
  - 대상 후보(시간 민감 순): market-trend 06:30 · 방산 05:40 · 건기 06:10 · etf 07:00 ·
    market-flow 10:00/13:00/15:40/16:40.
